...
export const AnimatedTestimonials = ({
  ...
  const hasImages = testimonials.some(t => t.src);

  return (
    <div className={cn("max-w-sm md:max-w-4xl mx-auto px-4 md:px-8 lg:px-12 py-20", className)}>
      <div className={cn("relative grid grid-cols-1 gap-20", hasImages && "md:grid-cols-2")}>
        {hasImages && (
            <div>
            <div className="relative h-80 w-full">
                <AnimatePresence>
                {rotations && testimonials.map((testimonial, index) => (
                    testimonial.src && (
                    <motion.div
                        key={testimonial.src}
                        initial={{
        ...
        <div className={cn("flex justify-between flex-col py-4", !hasImages && "col-span-1")}>
          <motion.div
            key={active}
            initial={{
              y: 20,
